#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# gbrain-schema-pack-bundled-ssot.sh   (VF-FIX-SP-SSOT)
#
# WHY THIS EXISTS
#   Upstream #1574 / #1726 (open, confirmed @ 4ee530f): the bundled-pack
#   registry disagrees across files — commands/schema.ts packPathByName knows
#   ONLY 'gbrain-base', so `schema use/show gbrain-base-v2` (and every lens
#   pack) prints "Unknown pack", and runList hardcodes a 2-entry display list.
#   The operator cannot select the base-v2 pack the whole fix targets.
#
# WHAT THIS PATCH DOES
#   (1) NEW file src/core/schema-pack/bundled.ts — the single source of truth
#       for the 7 bundled names. (2) FULL-FILE REPLACEMENT of
#       src/commands/schema.ts keyed to 4ee530f: runList consumes the SSOT
#       (shows all 7, was 2); runShow <name> AND runUse <name> route through
#       loadActivePack({perCall}) — the same resolver as the active path — so
#       named show/use (a) resolve ALL bundled packs (defaultPackLocator
#       already knows all 7) and (b) return the MERGED manifest, so
#       `schema show <name>` and `schema active` AGREE (the load-bearing
#       reroute the scope critique flagged). runUse now validates the FULLY
#       RESOLVED pack before writing config (refuses a pack whose merge fails).
#
# ANCHOR / GUARD STRATEGY
#   PRE-CLOBBER sha256 guard: commands/schema.ts must be pristine 4ee530f
#   (6db3cdb0…). Mismatch ⇒ exit 1 ⇒ build fails. bundled.ts is NEW (created
#   if absent; idempotent via marker). Idempotency: marker VF-FIX-SP-SSOT.
#   Post-write audit re-greps the marker in both files.
#   DEPENDS on gbrain-schema-pack-resolve-merge being applied FIRST (the
#   reroute only yields merged counts once the merge is live) — the Dockerfile
#   orders merge → yaml → ssot.
#
...
if [ -z "$GBRAIN_SRC" ]; then echo "[sp-ssot] ERROR: gbrain src tree not found." >&2; exit 1; fi
echo "[sp-ssot] target: $GBRAIN_SRC"
CMD="$GBRAIN_SRC/commands/schema.ts"
BUN="$GBRAIN_SRC/core/schema-pack/bundled.ts"
[ -f "$CMD" ] || { echo "[sp-ssot] ERROR: $CMD not found — layout changed. RE-POINT." >&2; exit 1; }
MARKER='VF-FIX-SP-SSOT'
if grep -qF "$MARKER" "$CMD" && [ -f "$BUN" ] && grep -qF "$MARKER" "$BUN"; then
  echo "[sp-ssot] ✓ already applied; no-op."; exit 0
fi
sha_of(){ sha256sum "$1" 2>/dev/null | awk '{print $1}' || shasum -a 256 "$1" | awk '{print $1}'; }
CMD_VANILLA_SHA="6db3cdb0b5e731890d52e13589a6f56258dc3e1a770a723bb385589181b34d4c"
GOT="$(sha_of "$CMD")"
if [ "$GOT" != "$CMD_VANILLA_SHA" ]; then
  echo "[sp-ssot] ERROR: commands/schema.ts drift — NOT the 4ee530f version." >&2
  echo "[sp-ssot]   expected $CMD_VANILLA_SHA" >&2
  echo "[sp-ssot]   got      $GOT" >&2
  echo "[sp-ssot]   RE-KEY THIS DROP-IN (UPGRADING_GBRAIN.md → 'Re-keying a full-file-replacement patch')." >&2
  exit 1
fi
echo "[sp-ssot] pre-clobber guard OK (commands/schema.ts is pristine 4ee530f)."
mkdir -p "$(dirname "$BUN")"
# ---- bundled.ts (NEW file, carries marker VF-FIX-SP-SSOT) ----
cat > "$BUN" <<'VF_PATCH_EOF'
// VF-FIX-SP-SSOT (#1574 / #1726) — single source of truth for the bundled
// schema-pack names. Upstream had FIVE disagreeing registries (load-active's
// 7-list, mutate's 3-set, commands/schema's 1 + 2-list display), so
// `schema use/show <bundled>` printed "Unknown pack" and `schema list`
// showed 2. All call sites now consume this list.
//
// PRIMARY = the static 7-list (correct in the embedded/compiled binary where
// a readdir of the source tree can return partial). Disk-scan is additive
// elsewhere (runList already scans ~/.gbrain/schema-packs separately).

export const BUNDLED_PACK_NAMES: ReadonlyArray<string> = [
  'gbrain-base',
  'gbrain-recommended',
  'gbrain-creator',
  'gbrain-investor',
...
/** Read-only bundled names (refuse mutation). Mirror of the 7-list as a Set. */
export const BUNDLED_PACK_NAME_SET: ReadonlySet<string> = new Set(BUNDLED_PACK_NAMES);
VF_PATCH_EOF
# ---- commands/schema.ts (corrected drop-in, carries marker VF-FIX-SP-SSOT) ----
cat > "$CMD" <<'VF_PATCH_EOF'
...
import { BUNDLED_PACK_NAMES } from '../core/schema-pack/bundled.ts'; // VF-FIX-SP-SSOT
...
function runList(_args: string[]): void {
  const bundled = [...BUNDLED_PACK_NAMES]; // VF-FIX-SP-SSOT (was 2 hardcoded)
...
async function runShow(args: string[]): Promise<void> {
  ...
  let manifest;
  if (packName) {
    // VF-FIX-SP-SSOT: route the named-pack path through loadActivePack
    // ({perCall}) — same resolver as the active path — so `schema show
    // <name>` (a) resolves ALL bundled packs (defaultPackLocator knows all
    // 7; packPathByName only knew gbrain-base, hence "Unknown pack"), and
    // (b) returns the MERGED manifest (extends resolved), so `show <name>`
    // and `active` agree. Falls back to a clear error on a genuinely
    // unknown pack.
    try {
      const pack = await loadActivePack({ cfg: loadConfig(), remote: false, perCall: packName });
...
async function runUse(args: string[]): Promise<void> {
  ...
  // VF-FIX-SP-SSOT: validate the FULLY RESOLVED pack (extends merged) via
  // loadActivePack({perCall}) — the same resolver the active path uses. This
  // (a) resolves bundled names like gbrain-base-v2 (packPathByName only knew
  // gbrain-base → "Unknown pack"), and (b) proves the merge resolves (a
  // child whose `description: |` zeroed its types, or whose extends fails,
  // is refused at activation rather than silently activating a broken pack).
  try {
    await loadActivePack({ cfg: loadConfig(), remote: false, perCall: packName });
  } catch (e) {
...
VF_PATCH_EOF
fail=0
grep -qF 'VF-FIX-SP-SSOT' "$CMD" || { echo "[sp-ssot] ERROR: marker absent in commands/schema.ts." >&2; fail=1; }
grep -qF 'VF-FIX-SP-SSOT' "$BUN" || { echo "[sp-ssot] ERROR: bundled.ts missing/marker absent." >&2; fail=1; }
[ "$fail" -eq 0 ] || exit 1
echo "[sp-ssot] ✓ patch applied (bundled.ts SSOT + schema.ts reroute)."
